        ...
        <div className="grid gap-6 lg:grid-cols-12">
          {project.gallery.map((image, index) => (
            <Reveal className={index === 0 ? "lg:col-span-8" : "lg:col-span-4"} delay={index * 0.06} key={`${image.src}-${index}`}>
              <div className={index === 0 ? "relative aspect-[16/10] overflow-hidden rounded-2xl border border-slate-200 shadow-lg" : "relative aspect-[4/5] overflow-hidden rounded-2xl border border-slate-200 shadow-lg"}>
