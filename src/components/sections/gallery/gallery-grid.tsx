      ...
      <div className="grid auto-rows-[14rem] gap-4 sm:grid-cols-2 sm:auto-rows-[18rem] lg:grid-cols-12 lg:auto-rows-[15rem]">
        {items.map((item, index) => (
          <button
            ...
            className={cn(
              "group relative overflow-hidden bg-navy text-left",
          ...
          >
            ...
            <span className="absolute inset-0 bg-gradient-to-t from-navy/80 via-transparent to-transparent opacity-80 transition-opacity group-hover:opacity-100" />
            <span className="absolute inset-x-0 bottom-0 block p-5 text-white"><span className="block text-[0.62rem] font-bold uppercase tracking-[0.15em] text-blue-200">{item.category}</span><span className="display-type mt-2 block text-2xl">{item.caption}</span></span>
          </button>
        ))}
      </div>
      <AnimatePresence>
