          ...
          {values.map((value, index) => {
            ...
            return (
              <Reveal
                ...
                key={value.title}
              >
                <div className="group relative flex h-full min-h-52 flex-col justify-between overflow-hidden rounded-2xl border border-slate-200 bg-white p-7 shadow-[0_10px_30px_rgba(15,23,42,0.05)] transform-gpu will-change-transform backface-hidden transition-all duration-300 ease-out hover:-translate-y-1.5 hover:border-cobalt/30 hover:shadow-[0_20px_46px_rgba(15,23,42,0.10)]">
                  <span aria-hidden="true" className="absolute left-0 top-7 h-10 w-1 rounded-r-full bg-cobalt/35 transition-all duration-300 ease-out group-hover:h-16 group-hover:bg-cobalt" />
                  <div>
                    <div className="flex items-start justify-between gap-4">
                      <span className="grid size-12 place-items-center rounded-full border border-cobalt/15 bg-cobalt/8 text-cobalt transform-gpu will-change-transform backface-hidden transition-all duration-300 ease-out group-hover:-translate-y-1 group-hover:scale-110 group-hover:border-cobalt/30 group-hover:bg-cobalt/15 group-hover:shadow-[0_8px_20px_rgba(21,94,239,0.15)]">
                        <Icon aria-hidden="true" className="size-5 transition-transform duration-300 ease-out group-hover:scale-105" strokeWidth={1.8} />
                      </span>
        ...
        <div className="mt-20">
          ...
          <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {values.map((val, idx) => {
              ...
              return (
                <Reveal
                  className={`h-full ${idx === values.length - 1 ? "lg:col-start-2" : ""}`}
                  delay={idx * 0.05}
                  key={val.title}
                >
                  <div className="group relative flex h-full min-h-52 flex-col justify-between overflow-hidden rounded-2xl border border-slate-200 bg-white p-7 shadow-[0_10px_30px_rgba(15,23,42,0.05)] transform-gpu will-change-transform backface-hidden transition-all duration-300 ease-out hover:-translate-y-1.5 hover:border-cobalt/30 hover:shadow-[0_20px_46px_rgba(15,23,42,0.10)]">
