  ...
  return (
    ...
    >
      <Container>
        ...
        <div className="mt-10 sm:mt-16 grid gap-px overflow-hidden rounded-2xl border border-slate-200 bg-slate-200 shadow-[0_22px_65px_rgba(15,23,42,0.09)] lg:grid-cols-3">
          {sisterConcerns.map((concern, index) => (
            <Reveal
              ...
              key={concern.slug}
            >
              <div className="group relative h-full min-h-120 sm:min-h-136 overflow-hidden bg-white">
                <Image
                ...
                <div className="absolute inset-0 flex flex-col justify-between p-6 sm:p-9 text-ink">
                  <div className="flex items-start justify-between">
                    ...
                    <a
                      ...
                      className="grid size-10 sm:size-11 place-items-center rounded-full border border-cobalt/25 bg-white/92 shadow-sm backdrop-blur-sm transition-[background-color,color,border-color,box-shadow] duration-300 hover:border-cobalt hover:bg-cobalt hover:text-white hover:shadow-[0_10px_24px_rgba(21,94,239,0.24)]"
                      href="/contact#quotation-form"
                    >
                      <ArrowUpRight aria-hidden="true" className="size-4 sm:size-5 transition-transform duration-300 group-hover:rotate-45" />
                    </a>
                  </div>
                  <div>
                    <p className="mb-2 sm:mb-3 text-[0.68rem] sm:text-xs font-bold uppercase tracking-[0.16em] text-cobalt">
                      {concern.discipline}
                    </p>
                    <h3 className="display-type text-2xl sm:text-3xl lg:text-4xl font-bold leading-tight">
                      {concern.name}
                    </h3>
                    <p className="mt-1.5 sm:mt-2 text-xs font-semibold text-cobalt/80">
                      {concern.headline}
                    </p>
                    <p className="mt-3 sm:mt-4 line-clamp-3 text-xs sm:text-sm leading-6 text-ink/68">
                      {concern.description}
                    </p>
                  </div>
                </div>
