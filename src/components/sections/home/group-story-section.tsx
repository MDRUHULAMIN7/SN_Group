...
export function GroupStorySection() {
  return (
    <section
      ...
      className="blueprint-grid-dark relative overflow-hidden border-t border-white/10 bg-black text-white section-space"
      id="about-group"
    >
      <BlueprintParallax />
      <Container className="relative z-10">
        <div className="grid gap-14 lg:grid-cols-12 lg:items-center">
          <Reveal className="lg:col-span-6">
            ...
            <div className="mt-6 space-y-4 text-base leading-8 text-white/75 sm:text-lg">
              ...
              <p>
                Over the years, the Group has developed a diversified portfolio through three core businesses: <span className="font-bold text-white">S.N Engineering &amp; Construction</span>, <span className="font-bold text-white">S.N Import &amp; Export</span>, and <span className="font-bold text-white">Mehrish Holdings</span>.
              </p>
          ...
          <Reveal className="relative lg:col-span-6" delay={0.12}>
            <div className="relative aspect-[4/3] overflow-hidden rounded-2xl border border-white/15 bg-navy shadow-2xl sm:aspect-[16/11]">
              <Image
              ...
              <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-r from-cobalt/95 to-navy/95 p-6 backdrop-blur-md text-white sm:p-8">
                <ArrowDownRight aria-hidden="true" className="mb-3 size-6 text-blue-300" />
        ...
        <div className="mt-16 grid border-y border-white/12 sm:grid-cols-3 lg:mt-24">
          {facts.map((fact, index) => (
            <Reveal
              className="border-b border-white/12 px-0 py-7 last:border-b-0 sm:border-b-0 sm:border-r sm:px-7 sm:last:border-r-0 lg:px-10"
              delay={index * 0.08}
              key={fact.label}
            >
              <p className="display-type text-5xl font-bold text-cobalt">{fact.value}</p>
              <p className="mt-2 text-sm font-semibold text-white/60">{fact.label}</p>
            </Reveal>
          ))}
        </div>
      </Container>
