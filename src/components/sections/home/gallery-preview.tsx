...
export function GalleryPreview() {
  ...
  return (
    <section className="overflow-hidden border-t border-white/10 bg-black text-white section-space">
      <Container>
        <div className="grid gap-8 lg:grid-cols-12 lg:items-end">
          <Reveal className="lg:col-span-4 lg:pb-10">
            ...
            <p className="mt-6 max-w-sm text-sm leading-7 text-white/58">Concrete, steel, drawings, hands and the quiet precision between them—moments from the work.</p>
          ...
          <div className="grid gap-8 lg:col-span-3">
            {[first, third].map((item, index) => (
              <Reveal delay={0.14 + index * 0.06} key={item.id}>
                <div className="relative aspect-[4/3] overflow-hidden"><Image alt={item.alt} className="object-cover transition-transform duration-700 hover:scale-105" fill sizes="(min-width: 1024px) 25vw, 100vw" src={item.src} /></div>
                <p className="mt-3 text-xs font-bold uppercase tracking-[0.12em] text-white/45">{item.caption}</p>
              </Reveal>
            ))}
          </div>
        </div>
